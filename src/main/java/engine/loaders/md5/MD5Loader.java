...
import engine.loaders.md5.anim.*;
...
/**
 * @author Jake stanger
 * Loads MD5 animated models into meshes
 * which can be rendered in-game.
 */
public class MD5Loader
{
	private static final String NORMAL_FILE_SUFFIX = "_normal";
	
	public static GameItem process(MD5Model model, Vector3f defaultColour) throws Exception
	{
		List<MD5Mesh> md5MeshList = model.getMeshes();
		List<Mesh> meshList = new ArrayList<>();
		
		for(MD5Mesh md5Mesh : model.getMeshes())
		{
			Mesh mesh = generateMesh(model, md5Mesh);
			handleTexture(mesh, md5Mesh, defaultColour);
			meshList.add(mesh);
		}
		
		Mesh[] meshes = new Mesh[meshList.size()];
	...
	private static Mesh generateMesh(MD5Model model, MD5Mesh mesh)
	{
		List<AnimVertex> vertices = new ArrayList<>();
		List<Integer> indices = new ArrayList<>();
		
		List<MD5Mesh.MD5Vertex> md5Vertices = mesh.getVertices();
	...
	private static Mesh createMesh(List<AnimVertex> vertices, List<Integer> indices)
	{
		...
		float[] normalsArr = Utils.floatListToArray(normals);
		int[] indicesArr = Utils.intListToArray(indices);
		int[] jointIndicesArr = Utils.intListToArray(jointIndices);
		float[] weightsArr = Utils.floatListToArray(weights);
	...
	/**
	 * Iterate over joints, calculate transformation matrix,
	...
	/**
	 * Processes each animation frame and returns a list
	 * of AnimatedFrame instances.
	 * @param md5Model
	 * @param animModel
	 * @param invJointMatrices
	 * @return
	 */
	...
	private static AnimatedFrame processAnimationFrame(MD5Model md5Model, MD5AnimModel animModel, MD5Frame frame, List<Matrix4f> invJointMatrices)
	{
		...
		for (int i = 0; i < numJoints; i++)
		{
			...
			//Get position and orientation of base frame element
